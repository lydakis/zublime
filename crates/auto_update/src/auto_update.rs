...
pub fn check(_: &Check, window: &mut Window, cx: &mut App) {
    if let Some(message) = option_env!("ZED_UPDATE_EXPLANATION") {
        drop(window.prompt(
            ...
            "Zublime was installed via a package manager.",
            Some(message),
            &["Ok"],
            cx,
        ));
...
pub fn release_notes_url(cx: &mut App) -> Option<String> {
    ...
    let url = match release_channel {
        ...
        ReleaseChannel::Nightly => {
            "https://github.com/lydakis/zublime/commits/nightly/".to_string()
        }
        ReleaseChannel::Dev => "https://github.com/lydakis/zublime/commits/main/".to_string(),
    };
    Some(url)
}
...
impl InstallerDir {
    async fn new() -> Result<Self> {
        let installer_dir = std::env::current_exe()?
            .parent()
            .context("No parent dir for Zublime.exe")?
            .join("updates");
...
impl AutoUpdater {
    ...
    fn new(current_version: Version, client: Arc<Client>, cx: &mut Context<Self>) -> Self {
        ...
        // to finish the auto update process after Zublime exits. When restarting
    ...
    // If you are packaging Zublime and need to override the place it downloads SSH remotes from,
    ...
    pub async fn download_remote_server_release(
        ...
        if smol::fs::metadata(&version_path).await.is_err() {
            log::info!(
                "downloading zublime-remote-server {os} {arch} version {}",
                release.version
            );
    ...
    async fn update(this: Entity<Self>, cx: &mut AsyncApp) -> Result<()> {
        ...
        let fetched_release_data =
            Self::get_release_asset(&this, release_channel, None, "zublime", OS, ARCH, cx).await?;
    ...
    async fn target_path(installer_dir: &InstallerDir) -> Result<PathBuf> {
        let filename = match OS {
            "macos" => anyhow::Ok("Zublime.dmg"),
            "linux" => Ok("zublime.tar.gz"),
            "windows" => Ok("Zublime.exe"),
...
async fn install_release_linux(
    ...
    let extracted = temp_dir.path().join("zublime");
    ...
    let app_folder_name = format!("zublime{}.app", suffix);

    let from = extracted.join(&app_folder_name);
    let mut to = home_dir.join(".local");

    let expected_suffix = format!("{}/libexec/zublime-editor", app_folder_name);
    ...
    anyhow::ensure!(
        output.status.success(),
        "failed to copy Zublime update from {:?} to {:?}: {:?}",
        from,
        to,
        String::from_utf8_lossy(&output.stderr)
    );
...
async fn install_release_macos(
    ...
    let mount_path = temp_dir.path().join("Zublime");
...
mod tests {
    ...
    async fn test_auto_update_downloads(cx: &mut TestAppContext) {
        ...
        dmg_tx.send("<fake-zublime-update>".to_owned()).unwrap();
        ...
        cx.update(|cx| {
            ...
            cx.set_global(InstallOverride(Rc::new(move |target_path, _cx| {
                let tmp_dir = tmp_dir.clone();
                let dest_path = tmp_dir.path().join("zublime");
