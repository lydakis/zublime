...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Zublime failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Zublime failed to open a window: {e:?}. See https://github.com/lydakis/zublime for troubleshooting steps."
    );
            ...
            proxy
                .add_notification(
                    ...
                    Notification::new("Zublime failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://github.com/lydakis/zublime for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("Zublime System Specs (from CLI):\n{}", system_specs);
        return;
    }
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Zublime`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zublime`.
    /// On Windows, the default is `%LOCALAPPDATA%\Zublime`.
    ...
    /// Used for recording minidumps on crashes by having Zublime run a separate
